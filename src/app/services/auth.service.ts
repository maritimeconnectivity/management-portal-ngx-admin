...
export class AuthService {
          ...
          for (let roleString of roles) {
            ...
            switch (role) {
              case RoleNameEnum[RoleNameEnum.ORGADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.OrgAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.SITEADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.SiteAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.USERADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.UserAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.DEVICEADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.DeviceAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.VESSELADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.VesselAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.SERVICEADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.ServiceAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.ENTITYADMIN]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.EntityAdmin;
                break;
              }
              case RoleNameEnum[RoleNameEnum.APPROVEORG]: {
                this.authState.permission =
                  this.authState.permission | AuthPermission.ApproveOrg;
                break;
              }
              default:
                this.authState.permission =
  ...
  private static parseAuthInfo(keycloakToken: any) {
    ...
    let permissions = keycloakToken.permissions
      ? keycloakToken.permissions
      : "";
    ...
    let authUser: AuthUser = {
      ...
      keycloakPermissions: permissions,
    };
  ...
  logout() {
    try {
      ...
      // State is somehow lost. Just do nothing.
    }
  }
  ...
  static getToken(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      if (AuthService.staticAuthInfo.authz.token) {
        AuthService.staticAuthInfo.authz
          .updateToken(30)
          .success(() => {
    ...
    try {
      ...
      AuthService.staticAuthInfo.authz.logout({
        redirectUri:
          ...
          "/#" +
          AuthService.staticAuthInfo.logoutUrl +
          "?reason=401",
      });
      AuthService.staticAuthInfo.authz = null;
    } catch (err) {
